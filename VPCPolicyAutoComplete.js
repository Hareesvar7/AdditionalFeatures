...
function provideVPCPolicyCompletionItems(document, position) {
    const linePrefix = document.lineAt(position).text.substr(0, position.character);

    // Trigger suggestions for VPC-specific rules
    ...
    msg = sprintf("Default security group in VPC '%s' is not closed", [resource.change.after.name])
        ...
        vpcSnippets.push(defaultSecurityGroupClosed);
    ...
    msg = sprintf("VPC '%s' does not have flow logs enabled", [resource.change.after.name])
        ...
        vpcSnippets.push(vpcFlowLogsEnabled);
    ...
    msg = sprintf("Network ACL '%s' is not in use", [resource.change.after.name])
        ...
        vpcSnippets.push(networkACLUnusedCheck);
    ...
    msg = sprintf("VPC Peering Connection '%s' does not allow DNS resolution", [resource.change.after.name])
        ...
        vpcSnippets.push(vpcPeeringDNSCheck);
    ...
    msg = sprintf("Security Group '%s' allows access to unauthorized port '%s'", [resource.change.after.name, resource.change.after.allowed_ports[_]])
        ...
        vpcSnippets.push(securityGroupAuthorizedPorts);

        // 6. Enforce Security Group Port Restriction Check
        const securityGroupPortRestriction = new vscode.CompletionItem('Enforce Security Group Port Restriction Check', vscode.CompletionItemKind.Snippet);
        securityGroupPortRestriction.insertText = new vscode.SnippetString(
            `deny[msg] {
    resource := input.resource_changes[_]
    resource.type == "aws_security_group"
    ...
    msg = sprintf("Security Group '%s' allows traffic on all ports (0)", [resource.change.after.name])
        ...
        vpcSnippets.push(securityGroupPortRestriction);
    ...
    msg = sprintf("VPN Connection '%s' does not have both tunnels up", [resource.change.after.name])
        ...
        vpcSnippets.push(vpnTunnelsStatusCheck);

        // Allow if no deny conditions are met
        const allowSnippet = new vscode.CompletionItem('Allow if no deny conditions are met', vscode.CompletionItemKind.Snippet);
        allowSnippet.insertText = new vscode.SnippetString(
            `allow {
    not deny[_]
}`
        );
        vpcSnippets.push(allowSnippet);
...
function registerVPCPolicyAutoCompletion(context) {
...
module.exports = {
    provideVPCPolicyCompletionItems,
    registerVPCPolicyAutoCompletion
};
